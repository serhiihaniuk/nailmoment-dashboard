...
describe("payment plan sync", () => {
  test.each(["free", "sponsor"] as const)(
    "zeroes finance and removes unpaid payments for %s plans",
    (paymentPlan) => {
      const payments = [
      ...
      expect(sync.financePatch).toEqual({
      ...
      expect(sync.expectedPaymentCount).toBe(0);
      expect(sync.targetPaymentCount).toBe(0);
      expect(sync.deletePaymentIds).toEqual(["payment-1", "payment-2"]);
      expect(sync.paymentPatches).toEqual([]);
      expect(sync.createPayments).toEqual([]);
      expect(projectPayments(payments, sync)).toEqual([]);
    }
  );

  test.each(["free", "sponsor"] as const)(
    "preserves paid payment history while zeroing %s plans",
    (paymentPlan) => {
      const paidDate = new Date("2026-02-01T10:00:00.000Z");
      const payments = [
        makePayment({ id: "payment-unpaid-1", installment_number: 1 }),
        makePayment({
          id: "payment-paid",
      ...
      expect(sync.paymentPatches).toEqual([
        { paymentId: "payment-paid", patch: { installment_number: 1 } },
      ]);
      expect(sync.createPayments).toEqual([]);
      expect(sync.targetPaymentCount).toBe(1);
  ...
  test.each([
    ...
    "preserves operator-managed payment structure for custom plans from $name tickets",
    ({ payments }) => {
      const sync = expectSync(
        buildPaymentPlanSync({
          finance: makeFinance(),
      ...
      expect(sync.financePatch).toEqual({ payment_plan: "custom" });
      expect(sync.expectedPaymentCount).toBeNull();
      expect(sync.targetPaymentCount).toBeNull();
      expect(sync.deletePaymentIds).toEqual([]);
      expect(sync.paymentPatches).toEqual([]);
      expect(sync.createPayments).toEqual([]);
      expect(projectPayments(payments, sync)).toEqual(payments);
    }
  );
