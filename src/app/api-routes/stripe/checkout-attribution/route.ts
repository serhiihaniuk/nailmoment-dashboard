...
import { checkoutAttributionClientSchema } from "@/shared/db/schema.zod";
...
export async function POST(request: NextRequest) {
  ...
  const parsed = checkoutAttributionClientSchema.safeParse(json.data ?? {});
  ...
  const attribution = await attributionService.upsertCheckoutAttribution(
    parsed.data
  );

  return jsonWithCors(
    origin,
    { attributionId: attribution.id, ok: true },
    { status: 201 }
  );
}
