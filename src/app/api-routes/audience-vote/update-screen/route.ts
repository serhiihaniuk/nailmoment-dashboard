...
import { parseAudienceVoteUpdateScreenSettings } from "@/entities/audience-vote";
...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      parseAudienceVoteUpdateScreenSettings(updateScreen),
      { status: 200 }
    );
  } catch (error) {
...
export async function PATCH(request: Request) {
  try {
    ...
    const updateScreen =
      await audienceVoteService.updateAudienceVoteUpdateScreen(parsed.data);
