...
| `GET /api/audience-vote/update-screen` | `app/api/audience-vote/update-screen/route.ts` | `src/app/api-routes/audience-vote/update-screen/route.ts` | Required | none | `parseAudienceVoteUpdateScreenSettings()` in audience vote page client | Reads the Operator-managed Audience Vote Update Screen content or the safe default. |
| `PATCH /api/audience-vote/update-screen` | `app/api/audience-vote/update-screen/route.ts` | `src/app/api-routes/audience-vote/update-screen/route.ts` | Required | `updateAudienceVoteUpdateScreenClientSchema` | `parseAudienceVoteUpdateScreenSettings()` in audience vote page client | Upserts simple Ukrainian update-screen content and optional button details. |
| `GET /api/audience-vote/mini-app` | `app/api/audience-vote/mini-app/route.ts` | `src/app/api-routes/audience-vote/mini-app/route.ts` | Telegram Mini App `initData`, not Better Auth | `x-telegram-init-data` or `Authorization: tma <initData>`; server HMAC validation with `TG_AUDIENCE_VOTE_BOT_TOKEN` | `parseAudienceVoteMiniAppResponse()` in Mini App page client | Upserts/reactivates the Telegram Voter and returns the open public feed or Operator-managed update screen plus next-vote context. |
