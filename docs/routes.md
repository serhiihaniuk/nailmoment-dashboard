...
| `GET /api/check-in/ticket` | `app/api/check-in/ticket/route.ts` | `src/app/api-routes/check-in/ticket/route.ts` | Dashboard `ticket.read` permission | none | `parseCheckInTicketList()` in check-in dashboard UI | Reads sanitized active ticket rows for entrance check-in only. |
| `GET /api/check-in/ticket/:id` | `app/api/check-in/ticket/[id]/route.ts` | `src/app/api-routes/check-in/ticket/[id]/route.ts` | Dashboard `ticket.read` permission | `ticketIdSchema` route param | `parseCheckInTicket()` in check-in ticket UI | Reads one sanitized active ticket row for QR/detail check-in. |
| `PATCH /api/check-in/ticket/:id/arrival` | `app/api/check-in/ticket/[id]/arrival/route.ts` | `src/app/api-routes/check-in/ticket/[id]/arrival/route.ts` | Dashboard `ticket.check-in` permission | `ticketIdSchema`, local `{ arrived }` schema | `parseCheckInTicket()` in check-in dashboard/detail UI | Toggles sanitized ticket arrival status without exposing admin ticket fields. |
